	...
	<?php
		...
		$nombrePersonal=$_REQUEST['nomPersonal'];

include "conexion.php";

$x= ("SELECT id_Personal FROM personal WHERE usuario_Personal='$nombrePersonal'");
$resultado =mysqli_query($conn, $x);
$value=mysqli_fetch_array($resultado);
$personales=$value['id_Personal'];
...
exit();
		...
		$consulta2="INSERT INTO pedidos (nombre_Pedidos,fecha_inicio_Pedidos,hora_inicio_Pedidos, fecha_final_pedidos, hora_final_pedidos, inventario_Pedidos,personal_pedidos, finalizado) VALUES
		 ('$objeto','$fecha_inicio','$hora_inicio','$fecha_final','$hora_final','$ido','$personales', '0')";
		 
		$consultafinal=mysqli_query($conn,$consulta2);
		header("location:reservahecha2.php");
